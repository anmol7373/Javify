...
function submitQuiz() {
    ...
    for (let i = 1; i <= totalQuestions; i++) {
        if (!answeredQuestions.has(i)) unansweredQuestions.push(i);
    }

    if (unansweredQuestions.length > 0) {
        // Show pop-up for unanswered questions
        showSubmissionPopup(
            `You have unanswered questions: ${unansweredQuestions.join(", ")}. Are you sure you want to submit?`,
            () => {
                isQuizOngoing = false; // Mark quiz as complete
                document.getElementById("quiz-form").submit(); // Submit the form
            }
        );
    } else {
        // Show pop-up for confirmation
        showSubmissionPopup("Are you sure you want to submit the quiz?", () => {
            isQuizOngoing = false; // Mark quiz as complete
            document.getElementById("quiz-form").submit(); // Submit the form
        });
    }
...
// Show the submission pop-up
function showSubmissionPopup(message, confirmCallback) {
    showPopup(message, confirmCallback);
}
